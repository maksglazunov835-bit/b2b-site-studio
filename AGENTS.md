...
- Keep live invocation `CODEX_ISOLATION_UNVERIFIED`. Windows named-profile canaries can read outside/junction targets and reach loopback; fresh CODEX_HOME removes legacy config but reports sandbox updateRequired, so equivalence/cause is not established. The owner authorized one separate `B2B-Codex-Lab` WSL2 Ubuntu lab, now installed without changing Docker, global WSL settings or Windows sandbox setup. Its partial command/exec canaries are not exec/tool-surface/cancellation admission. Linux login and full model receipt are absent; zero model calls. See `docs/design/wsl-lab.md` and the earlier `docs/design/astra-isolation.md`. Mount cleanup is per-boot, not a permanent boundary. Do not grant readiness from a profile name, zero exit, prior CI or partial matrix; never copy auth.json, alter user config, perform global setup, or remove the lab without appropriate authorization.
