...
- The installed Windows profile is currently `CODEX_ISOLATION_UNVERIFIED`: named elevated-profile canaries could read synthetic outside/junction targets and connect to loopback despite root deny/network disabled. Keep live invocation blocked. A separate WSL2 Ubuntu boundary with Windows drive mounts/interop disabled is proposed, not installed or proven; infrastructure/UAC/shared-permission changes require owner approval. See `docs/design/astra-isolation.md` for actual evidence and its limits. Never read/copy auth.json or change user config to force smoke success.
