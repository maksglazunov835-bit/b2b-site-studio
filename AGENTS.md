...
- MVP-04A adds only the separate `codex_design` / `design_proposal` profile 1.4: new project-scoped pairing and explicit per-job consent, one attempt and one pre-spawn invocation permission, 10-second lease / 180-second hard deadline. Previous presence/data credentials are never elevated. No website-file access, Git, publication or model-selected tools/commands. Model input is the six mapped fields of the pinned saved SiteSpec; output is a strict three-concept JSON rendered by owned components.
- The installed Codex 0.153.4 live profile is currently `CODEX_SAFE_PROFILE_UNVERIFIED`: effective empty tools/config isolation was not proven. Keep real invocation fail-closed. Never claim `test_stub` evidence is real Codex output or remove the safety gate just to pass smoke. Use existing official login without reading/copying auth.json, changing local config, or falling back to API keys. See `docs/design/README.md` for evidence and launch commands.
...
- Preserve the protected `design-regressions` gate on Linux and native Windows CI. Leader exit/pipe closure never proves tree termination; STOP_UNCONFIRMED forbids success/cancel acknowledgement. Keep incremental JSONL limits and discard reasoning/raw diagnostics. See `docs/design/profile-diagnostics.md` for the read-only 2026-09-09 policy matrix; app-server config receipts are not proof of an empty exec built-in tool set.
