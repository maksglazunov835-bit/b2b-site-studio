...
void test("clean migrations apply once and detect a changed checksum", async () => {
  ...
  assert.deepEqual(first.applied, ["001_initial_persistence.sql", "002_job_queue.sql", "003_agent_connections.sql"]);
  assert.deepEqual(first.skipped, []);

  const second = await runMigrations({ databaseConfig });
  assert.deepEqual(second.applied, []);
  assert.deepEqual(second.skipped, ["001_initial_persistence.sql", "002_job_queue.sql", "003_agent_connections.sql"]);
