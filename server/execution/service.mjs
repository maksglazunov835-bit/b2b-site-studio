...
export function createExecutionService({ workspaceId = DEFAULT_WORKSPACE_ID, clock = () => new Date() } = {}) {
  ...
  async function sweepIn(client, projectId = null, agentId = null) {
      ...
      for (const job of rows.rows) {
        ...
        const design = job.type === 'design_proposal';
        const exhausted = attempt.attempt >= (design ? DESIGN_POLICY : POLICY).maxAttempts;
        const failure = cancelled ? "STOP_UNCONFIRMED" : design ? 'INVOCATION_UNCERTAIN' : exhausted ? "ATTEMPTS_EXHAUSTED" : "LEASE_EXPIRED";
  ...
  return {
    ...
    async dispatch(projectId, jobId, input, key) {
      ...
      return transaction(async (client) => {
        ...
        const { grant } = await authorize(client, input.agentId, undefined, projectId);
        const job = await lockedJob(client, projectId, jobId);
        if (job.type !== grant.type) executionError('EXECUTION_SCOPE_MISMATCH', 403);
        ...
        const spec = job.type === 'design_proposal' ? materializeDesign(job, revision.canonical_site_spec, workspaceId, grant.runtime) : materialize(job, revision.canonical_site_spec, workspaceId);
        const now = clock();
    ...
    async claim(agentId, secret, input, key) {
      ...
      return transaction(async (client) => {
        ...
        const policy = job.type === 'design_proposal' ? DESIGN_POLICY : POLICY;
        (job.type === 'design_proposal' ? assertDesignSpec : assertSpec)(execution.job_spec, execution.spec_sha256.trim());
    ...
    async action(agentId, jobId, secret, kind, input, key) {
      assertAgentId(agentId); assertJobId(jobId);
      ...
      return transaction(async (client) => {
        ...
        executionRequest(kind, input, job.type === 'design_proposal');
        ...
        if (kind !== "heartbeat") {
          ...
          if (previous) {
            if (previous.request_sha256.trim() !== hash) executionError("IDEMPOTENCY_CONFLICT");
            if (kind === 'start' && job.type === 'design_proposal') executionError('INVOCATION_REPLY_UNAVAILABLE');
            return { ...previous.response, replayed: true };
          }
        }
        ...
        if (kind === "start") {
          ...
          if (job.type === 'design_proposal') await client.query('INSERT INTO design_invocations(job_id,attempt,consumed_at) VALUES($1,$2,$3)', [jobId,input.attempt,now]);
          ...
          (job.type === 'design_proposal' ? assertDesignSpec : assertSpec)(execution.job_spec, execution.spec_sha256.trim());
          const expected = job.type === 'design_proposal' ? assertDesignReport(input.report, execution.job_spec, input.attempt) : validationReport(execution.job_spec, input.attempt);
          ...
          await transition(client, job, "succeeded", "job_succeeded", now, { attempt: input.attempt, ...(job.type === 'design_proposal' ? { provider: expected.provider, providerInvocations: 1 } : { validationStatus: expected.validationStatus }), resultDigest: input.resultDigest });
